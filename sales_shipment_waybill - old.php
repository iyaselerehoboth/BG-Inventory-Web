...
$pdf->Cell(49,10,'',0,1);
$pdf->Cell(30,8,'PRODUCT ID',1,0,'C',0);
$pdf->Cell(75,8,'DESCRIPTION',1,0,'C',0);
$pdf->Cell(50,8,'SHIP FROM',1,0,'C',0);
//$pdf->Cell(41,8,'DESTINATION',1,0,'L',0);
$pdf->Cell(39,8,'QUANTITY LOADED',1,1,'C',0);

$anothersql = "SELECT quantity,product_id,description,ship_from,unit FROM user_waybill WHERE waybill_id = '$waybill_id'";
$anotherexe = mysqli_query($db_con, $anothersql);
if(!$anotherexe){
	die(mysqli_error($db_con));
}
while($anotherRun = mysqli_fetch_array($anotherexe)){
	$warehouseSQL = "SELECT warehousename FROM warehouse_t WHERE warehouseid = '".$anotherRun['ship_from']."'";
    ...
    $pdf->Cell(30,8,$anotherRun['product_id'],1,0,'C',0);
    $pdf->Cell(75,8,$anotherRun['description'],1,0,'C',0);
    $pdf->MultiCell(50,4,"   ".$anotherRun['ship_from']."    (".$warehouseEXEC['warehousename'].")",1,'C',false);
    $pdf->SetXY(165,109);
    $pdf->Cell(39,8,$anotherRun['quantity'].' '.$anotherRun['unit'],1,1,'C',0);
}
...
$pdf->Cell(49,10,'',0,1);
$pdf->Cell(98,10,'Loaded By General Services Supervisor ',1,0,'L',0);   // empty cell with left and top borders
$pdf->Cell(40,10,'Item QR Code ',0,2,'C'); 
$pdf->Image("ItemCodes/".$shipFrom['product_id'].".png",$pdf->GetX(),$pdf->GetY(),40,0,'PNG');
}
$pdf->SetX(10);
