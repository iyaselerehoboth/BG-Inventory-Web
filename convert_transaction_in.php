...
<H4 align = "center">Welcome <?php echo $_SESSION['login_user']?></H4>
       <!-- <h3>THe session level: <?php echo $_SESSION['sessionlevel']?></h3> -->
        ...
        Enter ItemID<input type="text" list="productdrop" name = "itemIDlist" id="itemIDlist" autocomplete="off" placeholder="Enter Item ID"  class="box"/>
                    ...
                    <p>
    Enter Warehouse In<input type="text" list="destinationdrop" name = "destinationlist" id="destinantionlist" autocomplete="off" placeholder="Enter Warehouse Name to return Item" onchange = "validatewarehouse(this.value);" class="box"/> <a id="warehouse_error" style="color:red;"></a>
    ...
    Enter Unit to bring in <input type="text" name="quantitylist" id="quantitylist" placeholder="Enter Item Unit" onchange="validateunit();" class="box"/> <a id="unit_error" style="color:red;"></a> <br>
...
<?php
    ...
    if(isset($_POST['create'])){
        ...
        $myregion = /*mysqli_real_escape_string($db_con,$_POST['regionlist'])*/ 'Conversion In';
        ...
        $numbersql = mysqli_query($db_con, "SELECT pc_number FROM warehouse_t WHERE warehousename = '" .$mydestination. "'");
        ...
        <?php   
            ...
            $sql_insert = "INSERT INTO web_transactions (transactionID,itemId,item,region,destination,unit,quantity,date,personnelName,status,sdstatus,sdpersonnel,pcstatus,transaction_type,comments,sync) VALUES ('$uniqueID','$myitemID','$myitem','$myregion','$mydestination','$myunit','$myquantity','$mydate','".$_SESSION['login_user']."','$mystatus','pending','$mysdpersonnel','pending','Conversion IN','$mycomment','$mysync')";
        
        $result_insert = mysqli_query($db_con,$sql_insert);
        if($result_insert){
            
            $curl = curl_init();
            $number = mysqli_real_escape_string($db_con, $_POST['sdname2']);  //2348175917337
            $from = "Inventory";
            $text = "CONVERT IN,".$uniqueID.",".$myregion.",".$mydestination.",".$myitem.",".$myquantity.",".$myunit;
        ...
        //Send text to the PC official.
            $pc_curl = curl_init();
            $from = "Inventory";
            $text = "Convertion In,".$uniqueID.",".$myregion.",".$mydestination.",".$myitem.",".$myquantity.",".$myunit;
