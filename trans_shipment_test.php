...
<div class="box box-primary">
  <div class="box-body box-profile">
    ...
    <div class="col-sm-8">
      <center>
        <head>
            <title>Trans Shipment Waybill</title>
      ...
      <div class="createtransactionbox">
        <form action = "" method = "post">
          <center>
            <label><H3 align = "center">Trans-Shipment</H3></label>
            <br>
            <fieldset>
              ...
              <div style="float: left; width: 320px;">
                <p>
                  Enter Ship From:<input type="text" list="regiondrop" name = "regionlist" id="regionlist" autocomplete="off" onchange="checkregion();" placeholder="Enter Warehouse to move from" class="box"/><a id="ship_error" style="color:red;"></a>
                  <datalist id="regiondrop">
                    <?php
                    $regionsql = mysqli_query($db_con, "SELECT DISTINCT warehousename,warehouseid FROM warehouse_t");
              ...
              <div style="float: right; width: 320px;">
                <p>
                  Confirm Ship From:<input type="text" list="regiondrop2" name = "regionlist2" id="regionlist2" autocomplete="off" onchange="checkregion();" placeholder="Enter Warehouse to move from" class="box"/>
                  <datalist id="regiondrop2">
                    <?php
                    $regionsql = mysqli_query($db_con, "SELECT DISTINCT warehousename,warehouseid FROM warehouse_t");
                    while($regionrow = $regionsql->fetch_assoc()){
                      ?>
              ...
              <div style="float: right; width: 320px;">
                Confirm Team Member ID:<input type="text" name="destinationlist2" id="destinationlist2" list="teamdrop" placeholder="Enter Team Member ID" class="box"/><br>
                <datalist id="teamdrop">
                  <?php
                  $staffsql = mysqli_query($db_con, "SELECT * FROM bulkbana_apps.es_staff");
                  while($staffrow = $staffsql->fetch_assoc()){
              ...
              <p>
                <div style="float: left; width: 320px;">
                  Enter Intended Location <input type="text" list="locationdrop" autocomplete="off" name="intended_location" id="intended_location" placeholder="Enter Intended Location" class="box"/> <br>
                  <datalist id="locationdrop">
                    <!-- Adding the IDs to the intended locations so as to act as a further confirmation of who they're delivering to. -->
                    <?php
              ...
              <p>
                <div style="float: right; width: 320px;">
                  Confirm Intended Location <input type="text" list="locationdrop" autocomplete="off" name="intended_location2" id="intended_location2" placeholder="Enter Intended Location" class="box"/> <br>
                  <datalist id="locationdrop">
                    <!-- Adding the IDs to the intended locations so as to act as a further confirmation of who they're delivering to. -->
                    <?php
                    $regionsql = mysqli_query($db_con, "SELECT * FROM warehouse_t");
              ...
              <div style="float: right; width: 320px;">
                Confirm Quantity <input type="number" min="0" step="1" name="quantitylist2" id="quantitylist2" placeholder="Enter Quantity" class="box"/> <br>
              </div>
...
<script>
  ...
  function checkregion(){
    ...
    var item = document.getElementById('regionlist');
    var confirm = document.getElementById('regionlist2');

    if(item.value != confirm.value){

      document.getElementById('ship_error').innerHTML = "Error: Ship from locations dont match";

    }else{
      document.getElementById('ship_error').innerHTML = "";
    }
  }
...
if(isset($_POST['create'])){
  ...
  if(empty($myitemID) || empty($myitem) || empty($regionName) || empty($myhub) || empty($myintended_location) || empty($myquantity) || empty($mypersonnelName) || empty($mydestination) || empty($myemail) || empty($myunit)){
    ...
    swal("Oops...", "All entries must be filled", "error");
    </script>
    <?php
    ...
    $q2 = "SELECT lmdid FROM lmd_t WHERE lmdid = '".$myregion[1]."'";
    $regioncheck = mysqli_query($db_con, $q1);
    if(mysqli_num_rows($regioncheck) < 1){
      //Not found in the database, check the lmd table.
      $regioncheck_2 = mysqli_query($db_con, $q2);
      if(mysqli_num_rows($regioncheck_2) < 1){
        //Use JS to display prompt then exit script.
        ?>
        <script type="text/javascript">
        swal("Oops...", "Ship from location not found in database. Kindly enter a valid location.", "error");
        </script>
        <?php
        exit();
      }
    ...
    $intendedQ1 = "SELECT warehouseid FROM warehouse_t WHERE warehouseid = '".$myintended[1]."'";
    ...
    if(mysqli_num_rows($intendedcheck1) < 1){
      //Not found in warehouseT, let's check lmdt
      if(mysqli_num_rows($intendedcheck2) < 1){
        //Also not found in lmdt, now lets alert and terminate script...
        ?>
        <script type="text/javascript">
        swal("Oops...", "Intended location not found in database. Kindly enter a valid location.", "error");
        </script>
        <?php
        exit();
      }
    ...
    if(mysqli_num_rows($staffcheck) < 1){
      ?>
      <script type="text/javascript">
      swal("Oops...", "Staff ID not found in database. Kindly enter a valid ID", "error");
      </script>
      <?php
      exit();
    }

    $trans_sql = "INSERT INTO user_waybill (waybill_id, description, product_id, ship_from, destination, delivery_address, intended_location, quantity, unit , batch_code , unit_of_measure ,emailAddress, date, personnelName, status, hub, transaction_type)
    VALUES ('$mywaybillId','$myitem','$myitemID','$myregion[1]','$mystaff[1]','N/A','$myintended[1]','$myquantity','$myunit','N/A','N/A','$myemail','$mydate','$mypersonnelName','$mystatus', '$myhub' ,'Trans-Shipment')";
    $trans_insert = mysqli_query($db_con, $trans_sql);


    ?>
    <script type="text/javascript">
    swal("Congrats", "Transaction has been created", "success");
    </script>
    <?php
  }
}
